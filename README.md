    ...
    - 빈 스코프
      ...
      - 웹 관련 스코프
        ...
        - request 스코프 : 동시에 HTTP 요청이 올 때 어떤 요청이 남긴 로그인지 구별할 때 자주 사용. 
          - 스프링 애플리케이션 실행시켜서 request 스코프 빈을 사용해야되면 오류가 발생한다. 스프링 시작 단계에서는 request가 들어오지 않았기 때문에 빈 생성되지 않았음. 생성되지도 않은 빈 사용시 오류 발생. 이때 프로바이더를 사용해서 문제를 해결할 수 있다.
          - 커스텀 로거에 각 요청을 구분하기 위한  requestURL을 저장하는 부분은 스프링 인터셉터나 서블릿 필터 같은 곳을 활용하는 것이 좋다.
          - 웹과 관련된 정보를 다루는 컨트롤러 계층과 달리 서비스 계층은 웹 기술에 종속되지 않고, 되도록이면 순수하게 유지하는 것이 유지보수관점에서 좋다.
        - 프록시 방식 사용 ScopedProxyMode : 가짜 프록시 클래스를 만들어두고 요청이 오기 전에도 가짜 프록시클래스를 다른 빈에 미리 주입해줄 수 있다. (CGLIB 라이브러리로 타겟의 클래스나 인터페이스를 상속받은 가짜 프록시 객체를 만들어서 주입, 실제 요청이 오면 그때 내부에서 원래의 진짜 빈을 요청한다.)
          - 이렇게 Provider 혹은 Proxy를 사용해서 객체 조회를 필요한 시점까지 지연시킬 수 있다. 애너테이션 변경만으로도 원본 객체를 프록시 객체로 대체할 수 있다.
          - 특별한 스코프는 꼭 필요한 경우에만 사용하는 것이 좋다. (유지보수 및 테스트)
